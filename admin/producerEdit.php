...
<?php
error_reporting(0);
if (!isset($_GET['MaNhaSanXuat']) || $_GET['MaNhaSanXuat'] == NULL) {
    echo "<script>window.location = 'producerList.php'</script>";
} else {
    $id = $_GET['MaNhaSanXuat'];
}
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit'])) {
    $updateProducer = $producer->update_producer($_POST, $id);
}
?>
<style>
            ...
            <div class="card shadow mb-4" style="min-height: 520px;display: flex;align-items: center;justify-content: center;">
                <form action="#" class="form-horizontal-custom mt-3 review-form-box" method="POST" role="form" enctype="multipart/form-data" style="margin-top: 0 !important;">
                    <div class="form-group" style="margin: 20px 0px;">
                        <?php
                        $producerEdit = $producer->get_details_producer($id);
                        if ($producerEdit) {
                            while ($result = $producerEdit->fetch_assoc()) {
                        ?>
                                <div class="col-lg-6 col-xl-6 col-md-6 col-sm-12 user-input-info max-width-fix">
                                    <div class="user-input__label custom_text">Mã nhà sản xuất</div>
                                    <input class="form-control user-input" readonly placeholder="<?php echo $result['MaNhaSanXuat'] ?>" style="text-align: center;" />
                                </div>
                    </div>
                    <div class="form-group" style="margin: 20px 0px;">
                        <div class="col-lg-6 col-xl-6 col-md-6 col-sm-12 user-input-info max-width-fix">
