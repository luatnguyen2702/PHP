...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit'])) {
    $addCategory = $category->add_category($_POST);
}
?>
<style>
            ...
            <div class="card shadow mb-4" style="min-height: 520px;display: flex;align-items: center;justify-content: center;">
                <form action="#" class="form-horizontal-custom mt-3 review-form-box" method="POST" role="form" enctype="multipart/form-data" style="margin-top: 0 !important;">
                    <div class="form-group">
                        <span class="label-admin" style="text-align: center;">Tên Danh Mục</span>
                        <div class="col-md-12">
                            <select class="form-control" name="MaDanhMuc">
                                <option value="">Chọn danh mục</option>
                                <?php
                                $type_list = $typeproduct->gettype();
                                if ($type_list) {
                                    while ($result_category = $type_list->fetch_assoc()) {
                                ?>
                                        <option value="<?php echo $result_category['MaDanhMuc'] ?>"><?php echo $result_category['TenDanhMuc'] ?></option>
                                <?php }
                                } ?>
                            </select>
                        </div>
