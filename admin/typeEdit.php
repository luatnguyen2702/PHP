...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit'])) {
    $updateType = $typeproduct->update_type($_POST, $id);
}
?>
<style>
            ...
            <div class="card shadow mb-4" style="min-height: 520px;display: flex;align-items: center;justify-content: center;">
                <form action="#" class="form-horizontal-custom mt-3 review-form-box" method="POST" role="form" enctype="multipart/form-data" style="margin-top: 0 !important;">
                    <div class="form-group" style="margin: 20px 0px;">
                        <?php
                        $typeEdit = $typeproduct->get_details_type($id);
                        if ($typeEdit) {
                            while ($result = $typeEdit->fetch_assoc()) {
                        ?>
                                <div class="col-lg-6 col-xl-6 col-md-6 col-sm-12 user-input-info max-width-fix">
                                    <div class="user-input__label custom_text">Mã danh mục</div>
                                    <input class="form-control user-input" readonly placeholder="<?php echo $result['MaDanhMuc'] ?>" style="text-align: center;" />
                                </div>
                    </div>
                    <div class="form-group" style="margin: 20px 0px;">
                        <div class="col-lg-6 col-xl-6 col-md-6 col-sm-12 user-input-info max-width-fix">
                            <div class="user-input__label custom_text">Tên danh mục</div>
                            <input class="form-control user-input" name="TenDanhMuc" type="text" value="<?php echo $result['TenDanhMuc'] ?>" style="text-align: center;" />
                        </div>
                    </div>
            <?php
                            }
